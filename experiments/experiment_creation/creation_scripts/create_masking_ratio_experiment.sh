...
BASE="/home/ubuntu/smiles_encoder/experiments/masking_ratios_2"
...
dataset="fabikru/pubchem_and_chembl-2025-randomized-smiles-cleaned"
size="5M"

# Define the masking ratios to test
masking_ratios=("0.1" "0.15" "0.2" "0.4" "0.5" "0.6" "0.7" "0.8" "0.9")

# Extract dataset key (everything after the last slash)
dataset_key="${dataset##*/}"

# Determine optimal learning rate (eta) and batch size for model 1M with this dataset
eta=0.032227
batch_size=4096
...
for masking in "${masking_ratios[@]}"; do
    ...
    # Define the new modern_bert_config block for the 5M model.
